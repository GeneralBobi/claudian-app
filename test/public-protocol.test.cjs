...
test('public generated content contains no owner biography and has a vault-independent protocol',()=>{
 for(const language of ['tr','en']){
  const skill=policy.skill('C:/Synthetic/Vault',['00 - Deniz (Hub).md'],language);
...
test('protocol upgrade preserves user notes and customized protocol byte for byte',async t=>{
 ...
 await setup.install((await setup.prepare({name:'Test User',vault,mode:'new',storage:'markdown',hosts:['claude-code'],language:'tr',access:'write'})).id,true);
 const files=['Decisions.md','Working Agreements.md','Vault Protocol.md','Personal project.md'];
 ...
 await fs.unlink(path.join(vault,'Vault Protocol.md'));
 await setup.upgrade();
 await assert.rejects(fs.access(path.join(vault,'Vault Protocol.md')));
 for(const [name,body]of originals)if(name!=='Vault Protocol.md')assert.equal(await fs.readFile(path.join(vault,name),'utf8'),body);
});
