...
test('modified skill fails safely without changing profile',async t=>{const{core,home}=await fixture(t);await core.removeHost('codex');await core.removeHost('cursor');await core.removeHost('gemini-cli');const skill=path.join(home,'.gemini/antigravity-cli/skills/claudian-memory.md');await fs.appendFile(skill,'\nPersonal edit');
 // Antigravity owns both entry points, so removing the provider walks into the edited file.
 // Nothing is removed: a half-removed provider would leave the user with a connection that
 // is listed as gone and still installed.
 await assert.rejects(core.removeHost('antigravity'),/modified/);assert.deepEqual((await core.snapshot()).profile.hosts.map(h=>h.id),['antigravity','antigravity-cli']);assert.match(await fs.readFile(skill,'utf8'),/Personal edit/);});
...
test('withdrawing every connection leaves nothing of Claudian in the home folder',async t=>{
 ...
 for(const id of input.hosts){if(!(await core.snapshot()).profile.hosts.some(h=>h.id===id))continue;await core.removeHost(id);}
