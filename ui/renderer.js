...
function remoteSettings(){
 return `<details class="connection cloud-settings" ${!remoteStatus?.relay?'open':''}><summary>${t('MCP connection settings','MCP bağlantı ayarları')}</summary><p>${t('Run your own MCP service in a terminal and enter its HTTPS relay address here. Existing device connections can continue with their saved address.','Kendi MCP hizmetini terminalde çalıştır ve HTTPS relay adresini buraya gir. Mevcut cihaz bağlantıları kayıtlı adresleriyle devam edebilir.')}</p><div id="remote-status">${remoteStatusBody()}</div>${!remoteStatus?.enabled&&remoteStatus?.relay?btn('Reconnect device','Cihazı yeniden bağla','connector-default',true):''}<label for="relay-url">${t('MCP relay address','MCP relay adresi')}</label><input id="relay-url" type="url" value="${esc(remoteStatus?.relay||'https://relay.claudian.app')}" ${remoteStatus?.enabled?'disabled':''}>${remoteStatus?.enabled?'':btn('Connect configured service','Tanımlanan hizmete bağlan','connector-start')}<div class="toolbar">${remoteStatus?.enabled?btn('Disconnect device','Cihaz bağlantısını kapat','connector-stop'):''}${btn('Refresh connection state','Bağlantı durumunu yenile','connector-refresh')}</div></details>`;
}
...
let resetArmed=false,resetResult=null;
const chatgptChoiceKey=()=>`claudian-chatgpt-method:${state?.profile?.vault||'default'}`;
function chatgptChoice(){try{return localStorage.getItem(chatgptChoiceKey())||'choose';}catch{return 'choose';}}
function setChatgptChoice(choice){try{localStorage.setItem(chatgptChoiceKey(),choice);}catch{}}
function chatgptChoiceCard(){
 return `<div class="connection"><h3>${t('Connect ChatGPT','ChatGPT bağlantısı')}</h3><p>${t('Choose how this computer will serve its memory to ChatGPT.','Bu bilgisayardaki hafızanın ChatGPT’ye nasıl sunulacağını seç.')}</p><div class="toolbar">${btn('Continue with Tunnel','Tunnel ile devam et','chatgpt-method',true,'data-method="tunnel"')}${btn('Continue with MCP / terminal','MCP / terminal ile devam et','chatgpt-method',false,'data-method="mcp"')}</div></div>`;
}
function tunnelCard(h){
 ...
 return `<div class="connection"><h3>${t('Personal Tunnel','Kişisel Tunnel')}</h3><p role="status">${esc(phase)}</p><p>${t('Your OpenAI account connects directly to the selected vault on this computer.','OpenAI hesabın doğrudan bu bilgisayardaki seçili vault’a bağlanır.')}</p>
 <details class="tunnel-settings"><summary>${s.hasKey?t('Tunnel settings','Tunnel ayarları'):t('Set up your tunnel','Tunnelini kur')}</summary>
 ...
 <p>${t('The key is encrypted outside the project.','Anahtar proje dışında şifrelenir.')}</p>
 ...
 <div class="toolbar">${btn('Save settings','Ayarları kaydet','tunnel-save',false,s.running?'disabled':'')}${btn('Remove saved key','Kayıtlı anahtarı kaldır','tunnel-forget')}${btn('OpenAI setup guide','OpenAI kurulum rehberi','tunnel-guide')}</div></details>
 <div class="toolbar">${s.running?btn('Stop Tunnel','Tunnel’i durdur','tunnel-stop',true):btn('Start Tunnel','Tunnel’i başlat','tunnel-start',true,s.hasKey?'':'disabled')}${btn('Refresh status','Durumu yenile','tunnel-refresh')}${btn('Change connection method','Bağlantı yöntemini değiştir','chatgpt-method',false,'data-method="choose"')}</div><p>${t('After starting, select your tunnel in ChatGPT and run the connection test.','Başlattıktan sonra ChatGPT’de tünelini seç ve bağlantı testini çalıştır.')}</p></div>`;
}
function webHostCard(h){
 if(h.id==='chatgpt'){
  const method=chatgptChoice();
  if(method==='choose')return chatgptChoiceCard()+memoryRow(h);
  if(method==='mcp')return (remoteStatus?.relay?`<div class="toolbar">${btn('Change connection method','Bağlantı yöntemini değiştir','chatgpt-method',false,'data-method="choose"')}</div>`:'')+legacyWebHostCard(h);
 ...
 return legacyWebHostCard(h);
}
function legacyWebHostCard(h){
 if(h.id==='chatgpt'&&!remoteStatus?.relay)return `<div class="connection"><h3>${t('MCP server / terminal','MCP sunucusu / terminal')}</h3><p>${t('Run your existing MCP server in a terminal or use a service you manage. Enter its HTTPS relay address below; an existing saved connection remains available.','Mevcut MCP sunucunu terminalde çalıştır veya yönettiğin bir hizmeti kullan. HTTPS relay adresini aşağıya gir; kayıtlı bağlantı kullanılmaya devam eder.')}</p>${btn('Change connection method','Bağlantı yöntemini değiştir','chatgpt-method',false,'data-method="choose"')}</div>`+memoryRow(h);
...
function mergedConnections(list){
 return list.filter(c=>visibleHost(c)&&!isVariant(c.id)).map(c=>{
...
function connectionUsable(id){
 if(id==='chatgpt')return tunnelState?.phase==='ready'||remoteStatus?.progress?.chatgpt?.canTest===true;
...
function connectionTile(h){
 const info=healthData?.hosts?.find(x=>x.id===h.id), review=reviewResults[h.id];
 const cloudReady=h.id==='chatgpt'?connectionUsable('chatgpt'):remoteStatus?.progress?.[h.id]?.canTest===true;
...
async function renderContent(){header();if(reviewing){renderReview();return;}
 ...
 if(view==='companion'){try{panelState=await api.state();}catch(e){error(e);}}const result=await (setup||extending?renderSetup():renderPanel());if(state.profile&&view==='settings'&&!extending&&!setup){content.insertAdjacentHTML('beforeend',`<section class="updates"><p>Claudian ${esc(state.appVersion)} · Protocol ${esc(state.profile.protocolVersion)}</p>${state.protocolConflicts?.length?`<div class="health-row health-warn"><div><span>${t('Protocol','Protokol')}</span><p>${t('Your memory protocol is older than this version and was left untouched because it differs from what Claudian installed.','Hafıza protokolün bu sürümden eski ve Claudian kurulumundan farklı olduğu için değiştirilmedi.')}</p><p class="path">${state.protocolConflicts.map(f=>esc(f.split(/[\\/]/).pop())).join(' · ')}</p></div>${btn('Install the current protocol','Güncel protokolü kur','adopt-protocol',true)}</div>`:''}${state.connectionConflicts?.length?`<div class="health-row health-warn"><div><span>${t('Connection files','Bağlantı dosyaları')}</span><p>${t('These files carry changes Claudian did not write, so they were left alone. Repair the connection to reinstall them.','Bu dosyalarda Claudian’ın yazmadığı değişiklikler var, bu yüzden dokunulmadı. Yeniden kurmak için bağlantıyı onar.')}</p><p class="path">${state.connectionConflicts.map(f=>esc(f.split(/[\/]/).pop())).join(' · ')}</p></div>${btn('Go to connections','Bağlantılara git','goto-connections')}</div>`:''}${btn('Check for updates','Güncellemeleri kontrol et','updates')}<span id="update-status" role="status"></span></section>`);content.insertAdjacentHTML('beforeend',resetSection());}return result;}
// Settings → remove every connection. Two steps: the first click only arms the button.
function resetSection(){
 const done=resetResult?`<p role="status">${t('Removed: ','Kaldırılan: ')}${resetResult.removed.length?esc(resetResult.removed.join(', ')):t('none','yok')} · ${t('Cloud permissions revoked: ','İptal edilen bulut izni: ')}${resetResult.revoked}${resetResult.failed.length?` · ${t('Could not remove: ','Kaldırılamayan: ')}${esc(resetResult.failed.map(f=>f.id+' ('+f.error+')').join(', '))}`:''}</p>`:'';
 return `<section class="updates"><h2>${t('Remove all connections','Tüm bağlantıları kaldır')}</h2><p>${t('Removes Claudian from every connected AI: its instructions, skills and connection files, the cloud permissions you granted, and this device’s remote connection. Your notes folder is not touched. Instructions you added to an AI account’s own memory must be deleted in that app.','Claudian’ı bağlı bütün AI’lardan kaldırır: yönergeleri, becerileri ve bağlantı dosyalarını, verdiğin bulut izinlerini ve bu cihazın uzak bağlantısını. Notlar klasörüne dokunulmaz. Bir AI hesabının kendi hafızasına eklediğin yönerge o uygulamanın içinden silinmelidir.')}</p>${resetArmed?`<p>${t('This cannot be undone from here; you would connect each AI again.','Bu işlem buradan geri alınamaz; her AI’ı yeniden bağlaman gerekir.')}</p><div class="toolbar">${btn('Yes, remove everything','Evet, hepsini kaldır','reset-all-confirm',true)}${btn('Cancel','Vazgeç','reset-all-cancel')}</div>`:btn('Remove all connections','Tüm bağlantıları kaldır','reset-all')}${done}</section>`;}
...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;selectedConnection=null;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 if(a==='chatgpt-method'){setChatgptChoice(el.dataset.method);await render();return;}
 ...
 if(a==='review-apply'){selectedHosts=expandSelection([...document.querySelectorAll('[name=review-host]:checked')].map(x=>x.value));
  // Hidden legacy Spark settings must not become an implicit removal.
  if((state.profile?.hosts||[]).some(h=>h.id==='gemini'))selectedHosts.push('gemini');
  busy=true;await render();try{const installed=(state.profile?.hosts||[]).map(h=>h.id);
  ...
  reviewResult=await api.reviewSetup(selectedHosts,granted,withdrawing);state=await api.snapshot();}finally{busy=false;}
  if(!reviewResult?.conflicts?.length){await api.finishReview();await api.enter('connections');return;}
  await render();return;}
 if(a==='review-done'){await api.finishReview();await api.enter('connections');return;}
 ...
 if(a==='add-hosts'){extending=true;plan=null;events=[];complete=false;const d=await api.discover();draft={name:state.profile.name,vault:state.profile.vault,storage:state.profile.storage,mode:'existing',action:'extend',language,hosts:d.suggested.hosts.filter(id=>id!=='gemini'&&!state.profile.hosts.some(h=>h.id===id))};await render();}
 ...
 if(a==='install'){if(!granted)return;busy=true;events=[];await render();try{await api.install(plan.id,true);state=await api.snapshot();}catch(e){busy=false;await render();throw e;}busy=false;await api.enter('connections');return;}
 ...
 if(a==='reset-all'){resetArmed=true;resetResult=null;await render();return;}
 if(a==='reset-all-cancel'){resetArmed=false;await render();return;}
 if(a==='reset-all-confirm'){resetArmed=false;resetResult=await api.resetAll();await render();return;}
...
(async()=>{language=(await api.preferences()).language;state=await api.snapshot();if(state.entryView)view=state.entryView;reviewing=!!state.setupReview;selectedHosts=(state.profile?.hosts||[]).map(h=>h.id);if(state.migrationError)error(new Error(state.migrationError));if(setup){draft=(await api.discover()).suggested;draft.hosts=draft.hosts.filter(id=>id!=='gemini');draft.language=language;}await render();})().catch(error);
...
function renderPanelView(){
 ...
 const attention=(s.attention||[]).filter(a=>a.host!=='gemini'),loops=s.openLoops||[];
 content.innerHTML=`<div class="panel-heading"><h1>Panel</h1><span class="panel-updated">${t('Updated','Güncellendi')}<time>${esc(when)}</time></span></div>`
  ...
  +(s.connections||[]).filter(visibleHost).map(c=>`<div class="panel-item"><div><strong>${esc(c.label)}</strong><p>${panelConnectionStatus(c)}</p></div>${btn('Open','Aç','open-next-connection',false,`data-host="${esc(c.id)}" aria-label="${esc(c.label)} ${t('connection','bağlantısını aç')}"`)}</div>`).join('')
  +`</section></aside></div>`;
...
function renderReview(){
 ...
 const hosts=state.hosts.filter(h=>visibleHost(h)&&!h.variantOf);
