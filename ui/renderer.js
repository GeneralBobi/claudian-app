...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;selectedConnection=null;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 ...
 if(a==='connector-provider'){await api.connectorProvider(el.dataset.host);remoteStatus=await api.connectorStatus();notice=el.dataset.host==='claude-desktop'?t('Claude opens with the name and address filled in. Review and confirm there.','Claude, adı ve adresi doldurulmuş olarak açılır. Orada kontrol edip onayla.'):t('Device address copied. Choose Create app and paste it into MCP server URL; authentication is OAuth.','Cihaz adresi kopyalandı. Create app / Uygulama oluştur seçeneğinde MCP sunucu URL alanına yapıştır; kimlik doğrulama OAuth.');await render();return;}
