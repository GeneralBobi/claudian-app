...
function appActions(h,copyAction,done=false){
 return `${btn('Copy instruction','Yönergeyi kopyala',copyAction)}${['codex','claude-code','claude-desktop','chatgpt','gemini','perplexity','antigravity','antigravity-cli','cursor'].includes(h.id)?btn('Copy and open app','Kopyala ve uygulamayı aç','open-in-ai',!done&&!cliReady[h.id],`data-host="${h.id}" data-copy="${copyAction}"`):''}`;
}
...
function webHostCard(h){
 const gemini=h.id==='gemini',url=remoteStatus?.urls?.[h.id];
 const grants=(remoteStatus?.grants||[]).filter(g=>g.host===h.id&&!g.revoked);
 const connected=grants.length>0;
 const requirement=gemini?t('Gemini web · Spark custom apps. Google currently limits this feature to eligible personal accounts in the US, age 18+, in English, with Spark access and Keep Activity on.','Gemini web · Spark özel uygulamaları. Google şu anda bu özelliği ABD’deki uygun kişisel hesaplarla, 18 yaş üstü kullanıcılarla ve İngilizceyle sınırlıyor; Spark erişimi ve Etkinliği sakla ayarı gerekiyor.'):t('Perplexity web · private remote connector. Your account must offer custom connectors; organization accounts may require administrator permission.','Perplexity web · kişisel uzak bağlantı. Hesabında özel connector seçeneği bulunmalı; kuruluş hesabında yönetici izni gerekebilir.');
 return `<div class="connection"><h2>${esc(h.label)}</h2><p>${requirement}</p>${connected?`<p class="health-ok">${checkIcon}${t('Account permission approved','Hesap izni onaylandı')}</p>`:`<p>${t('Prepare your device address and open the connection settings. Confirm access there.','Cihaz adresini hazırla ve bağlantı ayarlarını aç. Erişimi orada onayla.')}</p><div class="toolbar">${btn('Copy address and open setup','Adresi kopyala ve kurulumu aç','connector-provider',true,`data-host="${h.id}"`)}${btn('Set up with AI help','AI yardımıyla kur','connector-help',false,`data-host="${h.id}"`)}</div>`}
 <p>${gemini?t('In Connected Apps, add a custom app for Spark and paste the copied address.','Bağlı uygulamalarda Spark için özel uygulama ekle ve kopyalanan adresi yapıştır.'):t('Choose Custom connector → Remote. Name: Claudian. Authentication: OAuth. Transport: Streamable HTTP.','Özel connector → Remote seç. Ad: Claudian. Kimlik doğrulama: OAuth. Taşıma: Streamable HTTP.')}</p>
 ${connected?verifyRow({...h,access:{state:'ready',scope:grants.some(g=>g.scope?.includes('write'))?'write':'read'}})+firstScanRow(h):''}
 ${gemini?`<details><summary>${t('No Spark? Use Gemini with a guide','Spark yok mu? Gemini’yi yönergeyle kullan')}</summary><p>${t('The guide opens ordinary Gemini chat. It can work with excerpts you choose to share, but cannot connect to or update your local memory automatically. This does not complete the connection test.','Yönerge normal Gemini sohbetini açar. Seçerek paylaştığın içerikle çalışabilir; yerel hafızana kendiliğinden bağlanamaz veya yazamaz. Bu yöntem bağlantı testini tamamlamaz.')}</p>${btn('Copy guide and open Gemini','Yönergeyi kopyala ve Gemini’yi aç','gemini-web-guide')}</details>`:''}
 <details><summary>${t('Connection details','Bağlantı ayrıntıları')}</summary><p>${t('Keep Claudian and this computer running. Requested note content passes through the device relay.','Claudian ve bu bilgisayar açık kalsın. İstenen not içeriği cihaz relay hizmetinden geçer.')}</p>${url?`<p class="path">${esc(url)}</p>${btn('Copy address','Adresi kopyala','connector-copy',false,`data-host="${h.id}"`)}`:''}</details>${memoryRow(h)}</div>`;
}
function remoteHostCard(h){
 if(['gemini','perplexity'].includes(h.id))return webHostCard(h);
...
function connectionTile(h){
 ...
 const label=h.id==='gemini-cli'?t('Legacy CLI · not Gemini web','Eski CLI · Gemini web değil'):broken?t('Needs attention','Kontrol gerekli'):done?t('Completed','Tamamlandı'):verified?t('Review pending','İlk tarama bekliyor'):t('AI test pending','AI testi bekliyor');
