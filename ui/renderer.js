...
async function refreshRemote(){
 ...
 try{const before=JSON.stringify([remoteStatus?.urls,remoteStatus?.grants?.map(g=>[g.id,g.revoked])]);remoteStatus=await api.connectorStatus();const after=JSON.stringify([remoteStatus?.urls,remoteStatus?.grants?.map(g=>[g.id,g.revoked])]);if(before!==after)await render();else{const element=document.querySelector('#remote-status');if(element)element.innerHTML=remoteStatusBody();}}finally{remoteRefreshBusy=false;}
}
...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;selectedConnection=null;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 ...
 if(a==='connector-desktop-install'){await api.connectorDesktopInstall();notice=t('Complete the extension installation in Claude Desktop.','Claude Desktop penceresinde eklenti kurulumunu tamamla.');await render();return;}
