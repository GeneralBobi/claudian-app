...
function verifyStatus(){
 ...
 const say={waiting:t('Waiting for the AI to answer…','AI’ın cevabı bekleniyor…'),
 ...
 const retry=(verifyState==='timeout'||verifyState==='mismatch')&&challenge;
...
function verifyRow(h){
 ...
 return maintenanceRow(info)+`<div class="config-file verify-row" data-state="${state}"><span>${t('File access test','Dosya erişim testi')}</span><span class="badge">${esc(label)}${esc(when)}</span>
  ${open?`<p>${cliReady[h.id]?t('Claudian can open this session for you. Run it; the answer is awaited here. There is no command to type — the memory loads by itself.','Claudian bu oturumu senin için açabilir. Çalıştır; yanıt burada bekleniyor. Yazman gereken bir komut yok — hafıza kendiliğinden yükleniyor.'):t('Paste this into ','Şuraya yapıştır: ')+esc(h.label)+t('. The answer is awaited here; nothing else to press. There is no slash or @ command for this — the memory loads by itself at the start of a conversation.','. Yanıt burada bekleniyor, başka bir şeye basman gerekmiyor. Bunun için bir slash veya @ komutu yok — hafıza konuşmanın başında kendiliğinden yükleniyor.')}</p><pre class="prompt">${esc(challenge.prompt)}</pre><div class="toolbar">${cliReady[h.id]?btn('Run it now','Şimdi çalıştır','run-challenge',true,`data-host="${h.id}"`):btn('Copy instruction','Yönergeyi kopyala','copy-challenge')}${btn('Cancel','Vazgeç','challenge-close')}</div>${verifyStatus()}`
...
async function renderPanel(){const p=state.profile;
 ...
 if(autoChallenge&&hosts.length&&!challenge){autoChallenge=false;try{challenge={host:hosts[0].id,prompt:(await api.challenge(hosts[0].id)).prompt};}catch{}}
...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 ...
 if(a==='challenge-start'){verifyNotice='';verifyState='waiting';challenge={host:el.dataset.host,prompt:(await api.challenge(el.dataset.host)).prompt};api.verifyWatch(el.dataset.host).then(async r=>{if(r&&r.verified){challenge=null;verifyState='';healthData=await api.health();await render();}}).catch(()=>{});await render();return;}
 ...
 if(a==='challenge-close'){challenge=null;verifyNotice='';verifyState='';await render();return;}
 if(a==='run-challenge'){await api.scanSend(el.dataset.host,challenge.prompt);verifyState='waiting';verifyNotice='';await render();return;}
 if(a==='copy-challenge'){await api.copy(challenge.prompt);verifyState='waiting';verifyNotice='';await render();return;}
 if(a==='verify-run'){verifyState='waiting';verifyNotice='';await render();const r=await api.verifyWatch(el.dataset.host);verifyNotice=r.message;if(r.verified){challenge=null;verifyState='';healthData=await api.health();}await render();return;}
...
api.onVerify(async event=>{if(!challenge||event.host!==challenge.host)return;verifyState=event.state;if(event.message)verifyNotice=event.message;await render();});
