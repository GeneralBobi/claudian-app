...
let selfCheck=null;
let remoteStatus=null,remoteRefreshBusy=false;
...
function remoteSettings(){
 return `<section class="connection"><h2>${t('ChatGPT and Claude · web, desktop and mobile','ChatGPT ve Claude · web, masaüstü ve mobil')}</h2><p>${t('Keep this computer and Claudian running. Connect your AI account to this device; approve its request here. Your notes remain in your selected folder. Requests and responses pass through the relay service.','Bu bilgisayarı ve Claudian’ı açık tut. AI hesabını bu cihaza bağla; izin isteğini burada onayla. Notların seçili klasöründe kalır. İstekler ve yanıtlar relay hizmetinden geçer.')}</p><div id="remote-status">${remoteStatusBody()}</div><details><summary>${t('Relay service','Relay hizmeti')}</summary><label for="relay-url">${t('Trusted service address','Güvendiğin hizmetin adresi')}</label><input id="relay-url" type="url" placeholder="https://" value="${esc(remoteStatus?.relay||'')}" ${remoteStatus?.enabled?'disabled':''}></details><div class="toolbar">${remoteStatus?.enabled?btn('Disconnect device','Cihaz bağlantısını kapat','connector-stop'):btn('Connect device','Cihazı bağla','connector-start',true)}${btn('Refresh connection state','Bağlantı durumunu yenile','connector-refresh')}</div></section>`;
}
function remoteHostCard(h){
 const url=remoteStatus?.urls?.[h.id];
 const grants=(remoteStatus?.grants||[]).filter(g=>g.host===h.id&&!g.revoked);
 return `<div class="connection"><h2>${esc(h.label)}</h2><p>${esc(grants.length?t('Account authorization received on this device.','Bu cihazda hesap izni alındı.'):t('No account authorization received on this device yet.','Bu cihazda henüz hesap izni alınmadı.'))}</p><p>${t('1. Open the provider’s connectors and add Claudian using the address below. 2. Approve the matching code in Claudian. 3. Install the memory skill package and check its activation in the provider.','1. Sağlayıcının bağlantılarını açıp aşağıdaki adresle Claudian ekle. 2. Eşleşen kodu Claudian’da onayla. 3. Hafıza skill paketini kurup sağlayıcıda etkinliğini kontrol et.')}</p>${url?`<p class="path">${esc(url)}</p><div class="toolbar">${btn('Copy connector URL','Bağlantı adresini kopyala','connector-copy',false,`data-host="${h.id}"`)}${btn('Open account connections','Hesap bağlantılarını aç','connector-provider',true,`data-host="${h.id}"`)}${btn('Export memory plugin','Hafıza eklentisini hazırla','connector-export',false,`data-host="${h.id}"`)}</div>`:`<p>${t('Connect this device above to obtain its address.','Cihaza ait adresi almak için yukarıdan cihazı bağla.')}</p>`}<p>${t('A package on disk does not prove installation in the AI. OAuth authorization and actual AI calls are shown above separately.','Diskteki paket, AI içinde kurulduğunu kanıtlamaz. OAuth izni ve gerçek AI çağrıları yukarıda ayrı gösterilir.')}</p>${memoryRow(h)}${h.id==='claude-desktop'?`<details><summary>${t('Local Claude Desktop connection','Yerel Claude Desktop bağlantısı')}</summary>${selfCheckRow(h)}${accessRow(h)}${verifyRow(h)}</details>`:''}</div>`;
}
async function refreshRemote(){
 if(remoteRefreshBusy)return;remoteRefreshBusy=true;
 try{remoteStatus=await api.connectorStatus();const element=document.querySelector('#remote-status');if(element)element.innerHTML=remoteStatusBody();}finally{remoteRefreshBusy=false;}
}
...
async function renderPanel(){const p=state.profile;
 ...
 const hosts=await api.connections();healthData=await api.health();if(view==='connections')remoteStatus=await api.connectorStatus();
...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 ...
 if(a==='connector-copy'){await api.copy(remoteStatus.urls[el.dataset.host]);notice=t('Connector URL copied.','Bağlantı adresi kopyalandı.');await render();return;}
 if(a==='connector-provider'){await api.connectorProvider(el.dataset.host);return;}
 ...
 if(a==='copy-memory-trigger'){await api.copy(await api.memoryTrigger(el.dataset.host));notice=t('Instruction copied. Review it in your AI application; saving is not verified here.','Yönerge kopyalandı. AI uygulamanda incele; kaydedildiği burada doğrulanmaz.');await render();}
