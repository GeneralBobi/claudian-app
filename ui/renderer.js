...
document.addEventListener('click',async e=>{const nav=e.target.closest('[data-view]');if(nav&&!busy){view=nav.dataset.view;notice='';await render();return;}const el=e.target.closest('[data-action]');if(!el||busy&&el.dataset.action!=='cancel')return;el.disabled=true;errorBox.hidden=true;try{const a=el.dataset.action;
 ...
 if(a==='challenge-start'){verifyNotice='';verifyState='waiting';challenge={host:el.dataset.host,prompt:(await api.challenge(el.dataset.host)).prompt};const watchedHost=el.dataset.host;api.verifyWatch(watchedHost).then(async r=>{if(challenge?.host!==watchedHost)return;if(r&&r.verified){challenge=null;verifyState='';healthData=await api.health();await render();}}).catch(()=>{});await render();return;}
