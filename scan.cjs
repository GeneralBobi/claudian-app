...
Benim hakkımda zaten erişebildiğin bilgileri dikkatle gözden geçirmeni istiyorum. Yalnız bu oturumda gerçekten kullanılabilen hafızanı ve erişimine izin verdiğim bağlı kaynakları kullan. Önce hangi kaynaklara erişebildiğini kısaca belirt. Bir kaynak ek izin gerektiriyorsa veya kapsamı belirsizse okumadan önce sor. İlgisiz bilgisayar dosyalarını tarama; başka sağlayıcıların geçmiş sohbetlerine erişimin varmış gibi davranma. Faydalı bilgi yoksa bunu raporla ve taramayı tamamla. Bu kurulum taramasında kullanıcıya hedef, proje, tercih veya çalışma kuralı soruları yöneltme. Boş hafıza geçerli bir sonuçtur; yeni bilgiler normal kullanım sırasında birikir.
...
I want an initial, careful review of what you already know about me. Use only memories actually available to this session and connected sources I have authorized. First tell me briefly which sources you can access; if a source needs permission or its scope is unclear, ask before reading it. Do not scan unrelated computer files, invent access to earlier chats, or treat this request as access to other providers' accounts. If nothing useful is available, report that and complete the review. Do not ask the user onboarding questions about goals, projects, preferences or working rules. An empty vault is a valid result; new information accumulates during normal use.
